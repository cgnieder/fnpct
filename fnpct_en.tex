...
to footnotes: if footnote marks are followed by punctuation mark the order
...
\begin{beschreibung}
 \option{add-punct-mark}{<mark>[<space after>][<space before>]} add another punctuation
 mark for the position switching and spacify kerning if necessary. This option
 accepts list of marks.
 \option{remove-punct-mark}{<mark>} remove punctuation mark from the kerning/position
 ...
 \option{after-punct-space}{<dim>} set \emph{all} spaces.
 \option{punct-after}{\default{true}/false} when \code{true} the punctuation sign
 will be placed \emph{after} the footnote.
 ...
 \option{before-punct-space}{<dim>} set \emph{all} spaces.
...
\begin{beispiel}
 \setfnpct{add-punct-mark=![.03em]?[.03em]}
 \begin{minipage}{.4\linewidth}
  \noindent The three little pigs built their houses
  out of straw\footnote{not to be confused with hay}?
  Sticks\footnote{or lumber according to some sources}
