...
\begin{beschreibung}
 ...
 \Option{dont-mess-around}{\default{true}|false}\Default{false}
   ...
   footnote commands and \cmd{innernote} are still available, see
...
\begin{beschreibung}
 % \AdaptNote
 \Befehl{AdaptNote}{\cmd{cs}}\ma{\cmd{multcs}}\newline
   this lets \cmd{cs}  adapt to the kerning behaviour and adds an optional
   \code{*}. It also creates a corresponding mult-variant. The package option
   \key{multiple} will now turn \cmd{cs} into its mult-variant. Required syntax:
   \cmd{cs}[<oarg>]{<marg>}
 % \AdaptNoteNoMult
 \Befehl{AdaptNoteNoMult}{\cmd{cs}}\newline
   like \cmd{AdaptNote} but doesn't create a mult-variant and thus there is no
   change in behaviour when the option \key{multiple} is used. Required syntax:
   \cmd{cs}[<oarg>]{<marg>}
 % \AdaptNoteOpt
 \Befehl{AdaptNoteOpt}{\cmd{cs}}\ma{\cmd{multcs}}\newline
   this lets \cmd{cs} adapt to the kerning behaviour and adds an optional
   \code{*}. It also creates a corresponding mult-variant. The package option
   \key{multiple} will now turn \cmd{cs} into its mult-variant. Required syntax:
   \cmd{cs}[<oarg>]\oa{<oarg>}\ma{<marg>}
 % \AdaptNoteOptNoMult
 \Befehl{AdaptNoteOptNoMult}{\cmd{cs}}\newline
   like \cmd{AdaptNoteOpt} but doesn't create a mult-variant and thus there is
   no change in behaviour when the option \key{multiple} is used. Required syntax:
   \cmd{cs}[<oarg>]\oa{<oarg>}\ma{<marg>}
 % \AdaptNoteNoOpt
 \Befehl{AdaptNoteNoOpt}{\cmd{cs}}\ma{\cmd{multcs}}\newline
   \sinceversion{0.3}like \cmd{AdaptNote} but requires \cmd{cs} not to have an
   optional argument. If it had one it wouldn't have it any more after the adaption.
   The package option \key{multiple} will now turn \cmd{cs} into its mult-variant.
   Required syntax: \cmd{cs}{<marg>}
 % \AdaptNoteNoOptNoMult
 \Befehl{AdaptNoteNoOptNoMult}{\cmd{cs}}\newline
   \sinceversion{0.3}like \cmd{AdaptNoteNoOpt} but doesn't create a mult-variant.
   Required syntax: \cmd{cs}{<marg>}
 % \AdaptNoteMark
 \Befehl{AdaptNoteMark}{\cmd{cs}}\newline
   this lets \cmd{cs} adapt to the kerning behaviour and adds an optional
   \code{*}. Required syntax: \cmd{cs}[<oarg>]
 % strict
 \Option{strict}{\default{true}|false}\Default{false}
   when \code{true} \cmd{AdaptNote} (and the others) will issue errors if the
   note command in question has already been adapted.
\end{beschreibung}
