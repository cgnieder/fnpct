...
\begin{beschreibung}
 % \AdaptNote
 \Befehl{AdaptNote}{\cmd{cs}}\ma{\cmd{multcs}} this lets \cmd{cs}  adapt to
 ...
 % \AdaptNoteNoMult
 \befehl{AdaptNoteNoMult}{\cmd{cs}} like \cmd{AdaptNote} but doesn't create a
 ...
 % \AdaptNoteOpt
 \Befehl{AdaptNoteOpt}{\cmd{cs}}\ma{\cmd{multcs}} this lets \cmd{cs}  adapt to
 the kerning behaviour and adds an optional \code{*}. It also creates a
 corresponding mult-variant. The package option \key{multiple} will now turn
 \cmd{cs} into its mult-variant. Required syntax: \cmd{cs}[<oarg>]\oa{<oarg>}\ma{<marg>}
 % \AdaptNoteOptNoMult
 \befehl{AdaptNoteOptNoMult}{\cmd{cs}} like \cmd{AdaptNoteOpt} but doesn't create
 a mult-variant and thus there is no change in behaviour when the option \key{multiple}
 is used. Required syntax: \cmd{cs}[<oarg>]\oa{<oarg>}\ma{<marg>}
 % \AdaptNoteMark
 \befehl{AdaptNoteMark}{\cmd{cs}} this lets \cmd{cs} adapt to the kerning
 behaviour and adds an optional \code{*}. Required syntax: \cmd{cs}[<oarg>]
 % strict
